...
class DogWrapper(gym.Env):
    def __init__(self, task_name='walk'):
        ...
        self._move_speed = self._env.task._move_speed
        self.action_space = gym.spaces.Box(
            low=-1, high=1, shape=(self._env.action_spec().shape[0],), dtype=np.float64)
    ...
    def reset(self, seed: int | None = None, options: dict[str, Any] | None = None,):
        ...
        info = {'move_speed': self._move_speed}
        return obs, info
    ...
    def extract_obs(self, timestep):
        ...
        if self._task_name == 'multi':
            obs = np.append(obs, self._move_speed)
        return obs
