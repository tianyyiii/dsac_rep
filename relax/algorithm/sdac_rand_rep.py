...
from relax.network.sdac_rand_rep import SDACRandRepParams
...
class SDACRandRep(Algorithm):
        ...
        def stateless_update(
            ...
            (q1_params, q2_params, target_q1_params, target_q2_params,
              policy_params, target_policy_params, log_alpha,
                feat_params, target_feat_params, theta_params) = state.params
            ...
            def feature_loss_fn(params: hk.Params) -> jax.Array:
                ...
                mu = self.agent.mu(next_obs)
                
                norm_loss = jnp.mean(feat**2)
                prob_loss = jnp.mean(feat[:, None, :] @ mu[:, :, None])
                r_loss = 0.0
                if self.reward_loss_wgt > 0:
                    rhat = self.agent.theta(params['theta'], feat)
                    r_loss = jnp.mean((rhat - reward) ** 2)
                feature_loss = norm_loss - 2 * prob_loss + self.reward_loss_wgt * r_loss
                return feature_loss, (norm_loss, prob_loss, r_loss)

            feat_step_params = {'feature': feat_params, 'theta': theta_params}
            (feat_loss, (norm_loss, prob_loss, r_loss)), feat_grads = jax.value_and_grad(
            ...
            params = (policy_params, log_alpha, q1_params, q2_params, act_feat_params, theta_params) 
            ...
            state = SDACRepTrainState(
                params=SDACRandRepParams(q1_params, q2_params, target_q1_params, target_q2_params, policy_params, target_policy_params,
                                     log_alpha, feature_params, target_feat_params, theta_params),
    ...
    def get_policy_params(self):
        ...
        return (self.state.params.policy, self.state.params.log_alpha, self.state.params.q1, self.state.params.q2, feat_params, self.state.params.theta)

    def get_policy_params_to_save(self):
        return (self.state.params.target_policy, self.state.params.log_alpha, self.state.params.q1, self.state.params.q2, self.state.params.feature, self.state.params.theta)
