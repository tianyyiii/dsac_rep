...
from relax.network.diffurep_v2 import DiffURepNet, DiffURepParams
...
class DiffURep(Algorithm):
    ...
    def __init__(
        ...
        *,
        ...
        self.grn_optim = optax.adam(learning_rate=lr_schedule)
        ...
        self.state = DiffURepTrainState(
            ...
            opt_state=DiffURepOptStates(
                ...
                grn=self.grn_optim.init(params.grn),
        ...
        def stateless_update(
            ...
            q1_params, q2_params, target_q1_params, target_q2_params, policy_params, target_policy_params, phi_params, mu_params, grn_params, log_alpha = state.params
            q1_opt_state, q2_opt_state, policy_opt_state, phi_opt_state, mu_opt_state, grn_opt_state, log_alpha_opt_state = state.opt_state
            step = state.step
            ...
            next_action = self.agent.get_action(next_eval_key, (policy_params, log_alpha, q1_params, q2_params, phi_params, grn_params), next_obs)
            ...
            def get_feature_a(phi_params, grn_params, x, obs, t):
                ...
                feature = self.agent.phi(phi_params, obs, x, t)
                feature_a1= self.agent.grn(grn_params, feature)
                if len(obs.shape) == 2:
                    feature_a = jnp.reshape(feature_a1, (obs.shape[0], 256, -1))
                else:
                    feature_a = jnp.reshape(feature_a1, (256, -1))
            ...
            def policy_loss_fn(policy_params, phi_params, mu_params, grn_params) -> jax.Array:
                ...
                def denoiser(t, x):
                    _, feature_a = get_feature_a(phi_params=phi_params, grn_params=grn_params, x=x, obs=next_obs, t=t)
                ...
                phi_output, _ = get_feature_a(phi_params, grn_params, x_noisy, obs, t)
            ...
            (policy_loss, (rep_loss, q_weights, scaled_q, q_mean, q_std)), (policy_grads, phi_grads_p, mu_grads, grn_grads) = jax.value_and_grad(policy_loss_fn, argnums=(0, 1, 2, 3), has_aux=True)(policy_params, phi_params, mu_params, grn_params)
            ...
            phi_params, phi_opt_state = delay_param_update(self.phi_optim, phi_params, phi_grads, phi_opt_state)
            mu_params, mu_opt_state = delay_param_update(self.mu_optim, mu_params, mu_grads, mu_opt_state)
            grn_params, grn_opt_state = delay_param_update(self.grn_optim, grn_params, grn_grads, grn_opt_state)
            ...
            state = DiffURepTrainState(
                params=DiffURepParams(q1_params, q2_params, target_q1_params, target_q2_params, policy_params, target_policy_params, phi_params, mu_params, grn_params, log_alpha),
                opt_state=DiffURepOptStates(q1=q1_opt_state, q2=q2_opt_state, policy=policy_opt_state, phi=phi_opt_state, mu=mu_opt_state, grn=grn_opt_state, log_alpha=log_alpha_opt_state),
                step=step + 1,
                entropy=jnp.float32(0.0),
    ...
    def get_policy_params(self):
        return (self.state.params.policy, self.state.params.log_alpha, self.state.params.q1, self.state.params.q2, self.state.params.phi, self.state.params.grn)

    def get_policy_params_to_save(self):
        return (self.state.params.target_poicy, self.state.params.log_alpha, self.state.params.q1, self.state.params.q2, self.state.params.phi, self.state.params.grn)
