        ...
        *,
        ...
        self.state = Diffv2TrainState(
            ...
            opt_state=Diffv2OptStates(
                q1=self.optim.init(params.q1[0]),
                q2=self.optim.init(params.q2[0]),
        ...
        def stateless_update(
            ...
            q1_params, q1_state = q1_params
            q2_params, q2_state = q2_params
            target_q1_params, target_q1_state = target_q1_params
            target_q2_params, target_q2_state = target_q2_params

            reward *= self.reward_scale

            def get_min_q(s, a):
                nxt = self.agent.dynamics(s, a)
                q1, _ = self.agent.q(q1_params, q1_state, nxt)
                q2, _ = self.agent.q(q2_params, q2_state, nxt)
                q = jnp.minimum(q1, q2)
                return q

            next_action = self.agent.get_action(
                next_eval_key, (policy_params, log_alpha, (q1_params, q1_state), (q2_params, q2_state)), next_obs)
            nxtnxt = self.agent.dynamics(next_obs, next_action)
            q1_target, _ = self.agent.q(target_q1_params, target_q1_state, nxtnxt)
            q2_target, _ = self.agent.q(target_q2_params, target_q2_state, nxtnxt)
            # - jnp.exp(log_alpha) * next_logp
            q_target = jnp.minimum(q1_target, q2_target)
            q_backup = reward + (1 - done) * self.gamma * q_target

            def q_loss_fn(q_params: hk.Params, q_state: hk.Params) -> jax.Array:
                nxt = self.agent.dynamics(obs, action)
                q, _ = self.agent.q(q_params, q_state, nxt)
                q_loss = jnp.mean((q - q_backup) ** 2)
                return q_loss, q

            (q1_loss, q1), q1_grads = jax.value_and_grad(q_loss_fn, has_aux=True)(q1_params, q1_state)
            (q2_loss, q2), q2_grads = jax.value_and_grad(q_loss_fn, has_aux=True)(q2_params, q2_state)
            ...
            def policy_loss_fn(policy_params) -> jax.Array:
                ...
                t = jax.random.randint(
                    diffusion_time_key, (next_obs.shape[0],), 0, self.agent.num_timesteps)
            ...
            q1_params = (q1_params, q1_state)
            q2_params = (q2_params, q2_state)
            target_q1_params = (target_q1_params, target_q1_state)
            target_q2_params = (target_q2_params, target_q2_state)

            state = Diffv2TrainState(
                params=Diffv2Params(q1_params, q2_params, target_q1_params,
                                    target_q2_params, policy_params, target_policy_params, log_alpha),
                opt_state=Diffv2OptStates(
                    q1=q1_opt_state, q2=q2_opt_state, policy=policy_opt_state, log_alpha=log_alpha_opt_state),
                step=step + 1,
                entropy=jnp.float32(0.0),
