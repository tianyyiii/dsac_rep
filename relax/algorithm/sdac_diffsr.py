        ...
        *,
        ...
        def feature_step(state, data, key):
            def feature_loss_fn(params: hk.Params) -> jax.Array:
                ...
                noise = jax.random.normal(k1, next_obs.shape)
                t = jax.random.randint(k2, (next_obs.shape[0],), 0, self.agent.feature_num_timesteps)
                next_obs_noisy = jax.vmap(self.agent.feature_diffusion.q_sample)(t, next_obs, noise)
        
                feat = self.agent.feature(params['feature'], obs, action)
                mu = self.agent.mu(params['mu'], next_obs_noisy, t)
                noise_pred = jnp.squeeze(feat[:, None] @ mu)
                fdl = jnp.mean(jnp.square(noise_pred - noise))

                r_loss = 0.0
                if self.reward_loss_wgt > 0:
                    rhat = self.agent.theta(params['theta'], feat)
                    r_loss = jnp.mean((rhat - reward) ** 2)
                feature_loss = fdl + self.reward_loss_wgt * r_loss
                return feature_loss, (fdl, r_loss)
            ...
            (feat_loss, (fdl, r_loss)), feat_grads = jax.value_and_grad(
