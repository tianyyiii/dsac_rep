...
from relax.algorithm.sdac_rep import SDACRepTrainState, SDACRepOptStates

class SDACDiffSRPhi(Algorithm):

    def __init__(
        self,
        agent: SDACRepNet,
        params: SDACDiffSRParams,
        *,
        gamma: float = 0.99,
        lr: float = 1e-4,
        lr_feat: float = 1e-4,
        ...
        def feature_step(state, data, key):
            def feature_loss_fn(params: hk.Params) -> jax.Array:
                k1, k2 = jax.random.split(key)
                noise = jax.random.normal(k1, action.shape)
                t = jax.random.randint(k2, (action.shape[0],), 0, self.agent.feature_num_timesteps)
                action_noisy = jax.vmap(self.agent.feature_diffusion.q_sample)(t, action, noise)

                def compute_dot(action):
                    feat = self.agent.feature(params['feature'], obs, action, t)
                    mu = self.agent.mu(params['mu'], next_obs)
                    return jnp.sum(feat * mu)
                
                pred = jax.grad(compute_dot)(action_noisy)
                target = noise if self.predict_noise else action
            ...
            (q1_params, q2_params, target_q1_params, target_q2_params,
             policy_params, target_policy_params, log_alpha,
                feat_params, target_feat_params, 
                mu_params, target_mu_params, 
                theta_params, target_theta_params) = state.params
            ...
            feat_loss, feat_grads = jax.value_and_grad(feature_loss_fn)(feat_step_params)
            ...
            target_mu_params = delay_target_update(
                step, mu_params, target_mu_params, self.tau)
            target_theta_params = delay_target_update(
                step, theta_params, target_theta_params, self.tau)
            feat_metrics = {'total_feature_loss': feat_loss}
            return (feature_params, mu_params, theta_params, 
                    target_feat_params, target_mu_params, target_theta_params), feature_opt_state, feat_metrics
        ...
        def stateless_update(
            ...
            t_feat = jnp.zeros(obs.shape[0])
            if self.use_target_feature:
                feat = self.agent.feature(target_feat_params, obs, action, t_feat)
                next_feat = self.agent.feature(target_feat_params, next_obs, next_action, t_feat)
            else:
                feat = self.agent.feature(feat_params, obs, action, t_feat)
                next_feat = self.agent.feature(feat_params, obs, action, t_feat)
            ...
            state = SDACRepTrainState(
                params=SDACDiffSRParams(q1_params, q2_params, target_q1_params, target_q2_params, policy_params, target_policy_params,
                                     log_alpha, feature_params, target_feat_params, mu_params, target_mu_params, theta_params, target_theta_params),
        ...
        def update_aux(
            ...
            ((feature_params, mu_params, theta_params, target_feat_params, target_mu_params, target_theta_params),
                feature_opt_state, feat_metrics) = feature_step(state, data, key)
            state = SDACRepTrainState(
                params=SDACDiffSRParams(state.params.q1, state.params.q2,
                                     state.params.target_q1, state.params.target_q2, 
                                     state.params.policy, state.params.target_policy,
                                     state.params.log_alpha, feature_params, target_feat_params, 
    ...
    def get_policy_params_to_save(self):
        return (self.state.params.target_policy, self.state.params.log_alpha, self.state.params.q1, self.state.params.q2, self.state.params.target_feature, self.state.params.target_mu, self.state.params.target_theta)
