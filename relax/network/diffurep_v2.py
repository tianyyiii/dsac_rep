...
from relax.network.blocks import Activation, URepPhiNet, URepMuNet, URepQNet, URepPolicyNet, URepGradNet
...
class DiffURepNet:
    ...
    grn: Callable[[hk.Params, jax.Array], jax.Array]
    num_timesteps: int
    act_dim: int
    num_particles: int
    ...
    def get_action(self, key: jax.Array, policy_params: hk.Params, obs: jax.Array) -> jax.Array:
        policy_params, log_alpha, q1_params, q2_params, phi_params, grn_params = policy_params
        ...
        def model_fn(t, x):
            ...
            feature = self.phi(phi_params, obs, x, t)
            feature_a = self.grn(grn_params, feature)
            # return jax.lax.stop_gradient(self.policy(policy_params, feature_a))
            return self.policy(policy_params, feature_a)
    ...
    def get_deterministic_action(self, policy_params: hk.Params, obs: jax.Array) -> jax.Array:
        ...
        policy_params, log_alpha, q1_params, q2_params, phi_params, grn_params = policy_params
        log_alpha = -jnp.inf
        policy_params = (policy_params, log_alpha, q1_params, q2_params, phi_params, grn_params)
...
def create_diffurep_net(
    ...
    grn = hk.without_apply_rng(hk.transform(lambda feature: URepGradNet(activation=activation, embedding_dim=embedding_dim, act_dim=act_dim)(feature)))

    @jax.jit
    def init(key, obs, act, feature, feature_a):
        q1_key, q2_key, policy_key, phi_key, mu_key, grn_key = jax.random.split(key, 6)
        ...
        grn_params = grn.init(grn_key, feature)
        log_alpha = jnp.array(math.log(5), dtype=jnp.float32) # math.log(3) or math.log(5) choose one
        return DiffURepParams(q1_params, q2_params, target_q1_params, target_q2_params, policy_params, target_policy_params, phi_params, mu_params, grn_params, log_alpha)
    ...
    net = DiffURepNet(q=q.apply, policy=policy.apply, phi=phi.apply, mu=mu.apply, grn=grn.apply, num_timesteps=num_timesteps, act_dim=act_dim, 
