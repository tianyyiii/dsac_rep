...
from relax.network.blocks import Activation, DiffusionRepQNet, DiffusionRepPolicyNet, DiffusionRepMuNet
from relax.utils.diffusion import GaussianDiffusion
from relax.utils.jax_utils import random_key_from_data

# Note: Try two mu networks
class DiffRepParams(NamedTuple):
    q1: hk.Params
    q2: hk.Params
    target_q1: hk.Params
    target_q2: hk.Params
...
class DiffRepNet:
    ...
    def get_action(self, key: jax.Array, policy_params: hk.Params, obs: jax.Array) -> jax.Array:
        ...
        def sample(key: jax.Array) -> Union[jax.Array, jax.Array]:
            ...
            q1 = self.q(q1_params, obs, act)[1]
            q2 = self.q(q2_params, obs, act)[1]
            q = jnp.minimum(q1, q2)
    ...
    def get_smc_action(self, policy_params: hk.Params, obs: jax.Array) -> jax.Array:
        ...
        def model_q(action): 
            q1 = self.q(q1_params, obs_batched, action)[1]
            q2 = self.q(q2_params, obs_batched, action)[1]
            q = jnp.minimum(q1, q2)
            return q
...
def create_diffrep_net(
    ...
    q = hk.without_apply_rng(hk.transform(lambda obs, act: DiffusionRepQNet(hidden_sizes, activation)(obs, act)))
    ...
    def init(key, obs, act):
        ...
        return DiffRepParams(q1_params, q2_params, target_q1_params, target_q2_params, policy_params, target_policy_params, mu_params, log_alpha)
