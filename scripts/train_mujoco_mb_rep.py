...
from relax.algorithm.sdac_mb_rep import SDACMBRep
...
from relax.network.sdac_mb_rep import create_sdac_mb_rep_net
...
if __name__ == "__main__":
    ...
    parser.add_argument("--alg", type=str, default="sdac_mb_rep")
    ...
    parser.add_argument("--start_step", type=int,
                        default=int(1e3))  # other envs 3e4
    parser.add_argument("--total_step", type=int, default=int(1e5))  # 1e6
    ...
    assert args.alg == "sdac_mb_rep", "Only SDAC-Rep is supported in this script"

    def mish(x: jax.Array):
        return x * jnp.tanh(jax.nn.softplus(x))
    dyn_env = gym.make(args.env).unwrapped
    if not callable(getattr(dyn_env, 'dynamics', None)):
        raise ValueError("The environment does not have a dynamics function")
    agent, params = create_sdac_mb_rep_net(init_network_key, obs_dim=obs_dim, act_dim=act_dim, feature_dim=args.feat_dim,
                                           dynamics=dyn_env.dynamics, diffusion_hidden_sizes=diffusion_hidden_sizes,
                                        activation=mish, num_timesteps=args.diffusion_steps,
                                        num_particles=args.num_particles,
                                        noise_scale=args.noise_scale,
                                        target_entropy_scale=args.target_entropy_scale)
    algorithm = SDACMBRep(agent, params, lr=args.lr, alpha_lr=args.alpha_lr,
